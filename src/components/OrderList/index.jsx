            ...
            {orders && orders.map((order) => (
                <div key={order._id} className={styles.orderCard}>
                    <div className={styles.orderInfo}>
                        <h2>הזמנה מספר: {order._id}</h2>
                        <p><strong>שם לקוח:</strong> {order.customerId.name? order.customerId.name : null}</p>
                        <p><strong>דגם רכב:</strong> {order.carId.model}</p>
                        <p><strong>תאריך השכרה:</strong> {formatDate(order.from)} - {formatDate(order.to)}</p>
                        <p><strong>מחיר כולל:</strong> ₪{order.totalPrice.toLocaleString()}</p>
                    ...
                    {expandedOrder === order._id && (
                        <div className={styles.expandedDetails}>
                            <h3>פרטי לקוח</h3>
                            <p><strong>אימייל:</strong> {order.customerId.email}</p>
                            <p><strong>טלפון:</strong> {order.customerId.phone}</p>
                            <p><strong>רישיון נהיגה:</strong> {order.customerId.driverLicense}</p>
                            <p><strong>כתובת:</strong> {order.customerId.address.street} {order.customerId.address.houseNumber}, {order.customerId.address.city}</p>
                            ...
                            <p><strong>סוג דלק:</strong> {order.carId.fuelType}</p>
                            ...
                            <p><strong>הערות:</strong> {order.notes || 'אין הערות'}</p>
                        </div>
                    )}
                </div>
            ))}
