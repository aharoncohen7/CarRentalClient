...
const RentalList = ({rentals}) => {
  ...
  const handleChangeStatus = (orderId) => {
    console.log(`Change status for order ${orderId}`);
  };
  ...
  return (
    <div className={styles.orderList}>
      {rentals && rentals.map((order) => (
        <div key={order._id} className={styles.orderCard}>
          <div className={styles.orderInfo}>
            <p> {order._id}<strong>{' מספר הזמנה '}</strong></p>
            <p>{order.customerId.name}<strong>שם לקוח</strong> </p>
            <p> {order.carId.model}<strong>דגם רכב:</strong></p>
            ...
            <p> ₪{order.totalPrice}<strong>מחיר כולל</strong></p>
            <p> {order.status}<strong>סטטוס</strong></p>
          </div>
          <div className={styles.orderActions}>
            <button onClick={() => handleViewDetails(order._id)}>צפה בפרטים</button>
            <button onClick={() => handleChangeStatus(order._id)}>שנה סטטוס</button>
